      ...
      if (closest.length < numOfResults) {
        ...
        for (var i = 0; i < closest.length; i++) {
          ...
          if(newDiff < closest[i].diff) {
            ...
            closest.splice(i, 1);
            closest.push({ podcast: curr, diff: newDiff });
            // When removing the pod at index y the for loop through closest may not have finished, leading to other pods being added to closest when they are in fact much longer/shorter than appropriate. Might be fixed with a while loop?
            array.splice(y, 1);
            i = closest.length;
          }
        }
