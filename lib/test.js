...
function searchPodcasts(req, res) {
  ...
  .then(() => {
    ...
    .then(data => {
      ...
      .filter(Boolean)
      .map(channel => {
        // console.log('currently in episodes: ',channel.rss.channel.item);
        try{
      ...
      // console.log('why is it not sorting?', findClosest(jDuration, results[0].episodes));
      ...
      //
      //   try {
      //     return array.episodes.map(episode => {
      //       try {
      //         return {
      //           title: episode.title,
      //           duration: standardizeTime(episode['itunes:duration']),
      ...
      console.log(results[0].episodes);
