...
function searchPodcasts(req, res) {
  ...
  // console.log(Journey.findById(journeyId).origin);
  // // let originText = (journeyId)? Journey.findById(journeyId).origin : req.body.origin;
  // // let destinationText = (journeyId)? Journey.findById(journeyId).destination : req.body.destination;
  // let originText;
  // let destinationText;
  //
  // Journey
  ...
  const originText = req.body.origin;
  const destinationText = req.body.destination;
  const origin = originText.replace(/\s/g, '+') + '+london';
  const destination = destinationText.replace(/\s/g, '+') + '+london';
  ...
  .then((data) => {
    ...
    console.log(`https://itunes.apple.com/search?term=${topic}&media=podcast&limit=${limit}`);
    return rp({
      method: 'GET',
  ...
  .then(data => {
    return Promise.map(data.results, (podcast) => {
      console.log(podcast.feedUrl, 'podcast feed');
      return rp({
        method: 'GET',
        url: podcast.feedUrl,
        json: true
  ...
  .then(data => {
    ...
    res.render('journeys/show', { closest, journeyTime });
  })
  .catch(err => {
    console.log(err);
    return process.exit();
...
//Rewrite to split around colons and if length is 3 treat as first case, length is 2 treat as second case if no colons treat as seconds
