...
function searchPodcasts(req, res) {
  ...
  .then(() => {
    ...
    .then(data => {
      ...
      // there is a problem with certain podcast episode object formats that causes them to return undefined when 'tweaked'... (One example is The Art of Photography podcast)
      const standardizedEpisodes = results.map(result => {
        try {
      ...
      // const closest = [];
      // for (var i = 0; i < standardizedEpisodes.length; i++) {
      //   closest.push(sortByDiff(standardizedEpisodes[i]));
      // }


      console.log('Num of results', results.length);
      // const closest = findClosest(jDuration, standardizedEpisodes[0]);

      // console.log('input to findCLosest: ', standardizedEpisodes[0]);
      // console.log('closest: ', closest);
      const closest = sortByDiff(standardizedEpisodes);
      console.log('sorted podcast episodes: ', closest);
